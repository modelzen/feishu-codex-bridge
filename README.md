...
- **文档评论回复**：在飞书云文档（doc / docx / sheet / bitable 多维表格，含 wiki）的评论里 @ 机器人，它读评论、跑 agent、把答案回到同一条评论线程。 默认仅管理员 @ 可触发；管理员可在私聊管理台 → 全局设置 → 云文档评论，将「文档评论触发策略」改为「任何评论者 @」，允许外部协作者触发。详见 [评论触发策略](docs/comment-trigger-policy.md)。
