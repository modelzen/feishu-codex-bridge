...
- **🗑 删除** → 二次确认 → **解绑项目**（从注册表移除、撤销置顶横幅、之后不再在该群响应）→ **回一句提示"请你自行在飞书解散该群"**。bot 不退群、不解散（省去 `im:chat:delete` scope）；**绝不删代码目录**（仅"空白项目 + 私有 projects 目录 + 空目录"才额外询问删空目录）
- **⚙️ 设置** → 全局偏好卡（见 §6 / §5）
...
**✅ 已验证（飞书侧，用现有应用 `cli_aa81b50143785cd9` 实跑）**
